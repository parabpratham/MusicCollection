...
function handleDown(e) {
	...
	if (divX == 0) {
		divX = parseInt(sourceElement.style.left);
	}
	if (divY == 0) {
		divY = parseInt(sourceElement.style.top);
	}
	offsetX = e.x - parseInt(e.target.style.left);
	offsetY = e.y - parseInt(e.target.style.top);
	var player = document.getElementById("cont_bo_ltb_player_d");
	if (playerX == 0) {
...
function handleUp(e) {
	...
	var width = parseInt(player.style.width);
	var height = parseInt(player.style.height);
	...
	if (e.x > x && e.x < x + width && e.y > y && e.y < y + height) {
		...
		//XMLHttpRequestObject.onreadystatechange = function () {
			...
			//	document.getElementById("targetDiv5").innerHTML = XMLHttpRequestObject.responseText;
			//	delete XMLHttpRequestObject;
			//	XMLHttpRequestObject = null;
		//	}
		//};
	//	XMLHttpRequestObject.send(null);
		document.getElementById("cont_bo_ltb_playlist_item_d").innerHTML = "" + sourceElement.id;
	}
